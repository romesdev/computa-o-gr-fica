...
bool glutGUI::obliqua = false;
bool glutGUI::ortografica = false;

float glutGUI::anguloObliqua = 30;
...
void glutGUI::autoCamMove(float value, Axis axis, int nIterations) {
    if (axis == AXIS_Z && value < 0 && cam->e.getDistance(cam->c) <= 1.001) return;
    if (axis == AXIS_Y && fabs(cam->u.y) <= 0.1) return;
